...
const action = require("../../../actions/ODE/get-container");
...
describe("get-container - with sandbox", () => {
  const fakeParams = {
    __ow_headers: {
  ...
  const mockFetchResponse = {
    ok: true,
    json: () => Promise.resolve(mock.data.container),
  };
  ...
  test("missing input request parameters, should return 400", async () => {
    ...
    expect(response).toEqual({
      error: {
        ...
        body: {
          error:
            "missing header(s) 'authorization,x-gw-ims-org-id' and missing parameter(s) 'apiKey'",
        },
      },
    });
  });
});
describe("get-container - with out", () => {
  const fakeParams = {
    __ow_headers: {
      authorization: "Bearer fake",
      "x-gw-ims-org-id": "fake IMS ORG",
