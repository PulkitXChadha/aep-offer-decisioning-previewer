...
const action = require("../../../actions/ODE/get-offer-representation");
...
describe("get-offer-representation", () => {
  ...
  const mockFetchResponse = {
    ok: true,
    json: () => Promise.resolve(mock.data.offerRepresentationImage),
  };
  ...
  test("missing input request parameters, should return 400", async () => {
    ...
    expect(response).toEqual({
      error: {
        ...
        body: {
          error:
            "missing header(s) 'authorization,x-gw-ims-org-id' and missing parameter(s) 'containerID,apiKey,activityID,placementID'",
        },
      },
    });
  });
