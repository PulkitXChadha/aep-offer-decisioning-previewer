...
const action = require("../../../actions/UPS/get-identity-preview-report");
...
describe("get-identity-preview-report", () => {
  ...
  const mockFetchResponse = {
    ok: true,
    json: () => Promise.resolve(mock.data.identityPreviewReport),
  };
  ...
  test("missing input request parameters, should return 400", async () => {
    ...
    expect(response).toEqual({
      error: {
        ...
        body: {
          error:
            "missing header(s) 'authorization,x-gw-ims-org-id' and missing parameter(s) 'apiKey,sandboxName'",
        },
      },
    });
  });
